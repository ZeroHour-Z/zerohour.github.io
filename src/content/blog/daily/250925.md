...
在不牺牲线上延迟与成本的前提下，让推荐系统“吃到”大模型的红利，正成为主流工程路径。KDD'25 对 LLM-Enhanced Recommender Systems（LLMERS）的定义是：使用 LLM 在训练/数据侧进行增强，**服务阶段不需要 LLM 在线推理**，以保证低延迟与高吞吐（与端到端 LLM 在线推荐区分开）。这与本文的定位完全一致。
